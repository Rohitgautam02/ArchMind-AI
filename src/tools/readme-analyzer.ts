...
export interface ReadmeAnalysisResult {
  ...
  readonly installationInstructions: readonly string[];
...
export class ReadmeAnalyzer implements Tool<MetadataResult, ReadmeAnalysisResult> {
  ...
  execute(input: MetadataResult): ToolExecutionResult<ReadmeAnalysisResult> {
  ...
  #findReadme(rootPath: string): string | undefined {
  ...
  #parseReadme(content: string): ReadmeAnalysisResult {
  ...
  #extractTitle(lines: readonly string[]): string {
    const heading = lines.find((line) => /^#\s+/.test(line));
    return heading ? heading.replace(/^#\s+/, '').trim() : 'README';
  }

  #extractSections(lines: readonly string[]): readonly ReadmeSection[] {
  ...
  #extractTechnologies(content: string): readonly string[] {
  ...
  #extractInstallationInstructions(sections: readonly ReadmeSection[], content: string): readonly string[] {
  ...
  #extractUsageSection(sections: readonly ReadmeSection[], content: string): string {
